    function targMap = targDataMap(),

    ;%***********************
    ;% Create Parameter Map *
    ;%***********************
    
        nTotData      = 0; %add to this count as we go
        nTotSects     = 4;
        sectIdxOffset = 0;

        ;%
        ;% Define dummy sections & preallocate arrays
        ;%
        dumSection.nData = -1;
        dumSection.data  = [];

        dumData.logicalSrcIdx = -1;
        dumData.dtTransOffset = -1;

        ;%
        ;% Init/prealloc paramMap
        ;%
        paramMap.nSections           = nTotSects;
        paramMap.sectIdxOffset       = sectIdxOffset;
            paramMap.sections(nTotSects) = dumSection; %prealloc
        paramMap.nTotData            = -1;

        ;%
        ;% Auto data (realtsim_P)
        ;%
            section.nData     = 1;
            section.data(1)  = dumData; %prealloc

                    ;% realtsim_P.sens
                    section.data(1).logicalSrcIdx = 0;
                    section.data(1).dtTransOffset = 0;

            nTotData = nTotData + section.nData;
            paramMap.sections(1) = section;
            clear section

            section.nData     = 19;
            section.data(19)  = dumData; %prealloc

                    ;% realtsim_P.K
                    section.data(1).logicalSrcIdx = 1;
                    section.data(1).dtTransOffset = 0;

                    ;% realtsim_P.Ki
                    section.data(2).logicalSrcIdx = 2;
                    section.data(2).dtTransOffset = 4;

                    ;% realtsim_P.Nu
                    section.data(3).logicalSrcIdx = 3;
                    section.data(3).dtTransOffset = 5;

                    ;% realtsim_P.Nx
                    section.data(4).logicalSrcIdx = 4;
                    section.data(4).dtTransOffset = 6;

                    ;% realtsim_P.V2deg
                    section.data(5).logicalSrcIdx = 5;
                    section.data(5).dtTransOffset = 10;

                    ;% realtsim_P.deg2rad
                    section.data(6).logicalSrcIdx = 6;
                    section.data(6).dtTransOffset = 11;

                    ;% realtsim_P.rads2rpm
                    section.data(7).logicalSrcIdx = 7;
                    section.data(7).dtTransOffset = 12;

                    ;% realtsim_P.rpm2rads
                    section.data(8).logicalSrcIdx = 8;
                    section.data(8).dtTransOffset = 13;

                    ;% realtsim_P.AnalogOutput_FinalValue
                    section.data(9).logicalSrcIdx = 9;
                    section.data(9).dtTransOffset = 14;

                    ;% realtsim_P.AnalogOutput_InitialValue
                    section.data(10).logicalSrcIdx = 10;
                    section.data(10).dtTransOffset = 15;

                    ;% realtsim_P.EncoderInput_InputFilter
                    section.data(11).logicalSrcIdx = 11;
                    section.data(11).dtTransOffset = 16;

                    ;% realtsim_P.EncoderInput_MaxMissedTicks
                    section.data(12).logicalSrcIdx = 12;
                    section.data(12).dtTransOffset = 17;

                    ;% realtsim_P.AnalogInput_MaxMissedTicks
                    section.data(13).logicalSrcIdx = 13;
                    section.data(13).dtTransOffset = 18;

                    ;% realtsim_P.AnalogOutput_MaxMissedTicks
                    section.data(14).logicalSrcIdx = 14;
                    section.data(14).dtTransOffset = 19;

                    ;% realtsim_P.EncoderInput_YieldWhenWaiting
                    section.data(15).logicalSrcIdx = 15;
                    section.data(15).dtTransOffset = 20;

                    ;% realtsim_P.AnalogInput_YieldWhenWaiting
                    section.data(16).logicalSrcIdx = 16;
                    section.data(16).dtTransOffset = 21;

                    ;% realtsim_P.AnalogOutput_YieldWhenWaiting
                    section.data(17).logicalSrcIdx = 17;
                    section.data(17).dtTransOffset = 22;

                    ;% realtsim_P.IntervalTest_lowlimit
                    section.data(18).logicalSrcIdx = 18;
                    section.data(18).dtTransOffset = 23;

                    ;% realtsim_P.IntervalTest_uplimit
                    section.data(19).logicalSrcIdx = 19;
                    section.data(19).dtTransOffset = 24;

            nTotData = nTotData + section.nData;
            paramMap.sections(2) = section;
            clear section

            section.nData     = 7;
            section.data(7)  = dumData; %prealloc

                    ;% realtsim_P.EncoderInput_Channels
                    section.data(1).logicalSrcIdx = 20;
                    section.data(1).dtTransOffset = 0;

                    ;% realtsim_P.AnalogInput_Channels
                    section.data(2).logicalSrcIdx = 21;
                    section.data(2).dtTransOffset = 1;

                    ;% realtsim_P.AnalogOutput_Channels
                    section.data(3).logicalSrcIdx = 22;
                    section.data(3).dtTransOffset = 3;

                    ;% realtsim_P.AnalogInput_RangeMode
                    section.data(4).logicalSrcIdx = 23;
                    section.data(4).dtTransOffset = 4;

                    ;% realtsim_P.AnalogOutput_RangeMode
                    section.data(5).logicalSrcIdx = 24;
                    section.data(5).dtTransOffset = 5;

                    ;% realtsim_P.AnalogInput_VoltRange
                    section.data(6).logicalSrcIdx = 25;
                    section.data(6).dtTransOffset = 6;

                    ;% realtsim_P.AnalogOutput_VoltRange
                    section.data(7).logicalSrcIdx = 26;
                    section.data(7).dtTransOffset = 7;

            nTotData = nTotData + section.nData;
            paramMap.sections(3) = section;
            clear section

            section.nData     = 18;
            section.data(18)  = dumData; %prealloc

                    ;% realtsim_P.Out_Y0
                    section.data(1).logicalSrcIdx = 27;
                    section.data(1).dtTransOffset = 0;

                    ;% realtsim_P.DiscreteTimeIntegrator_gainval
                    section.data(2).logicalSrcIdx = 28;
                    section.data(2).dtTransOffset = 1;

                    ;% realtsim_P.DiscreteTimeIntegrator_IC
                    section.data(3).logicalSrcIdx = 29;
                    section.data(3).dtTransOffset = 2;

                    ;% realtsim_P.uV_Y0
                    section.data(4).logicalSrcIdx = 30;
                    section.data(4).dtTransOffset = 3;

                    ;% realtsim_P.TransferFcn_A
                    section.data(5).logicalSrcIdx = 31;
                    section.data(5).dtTransOffset = 4;

                    ;% realtsim_P.TransferFcn_C
                    section.data(6).logicalSrcIdx = 32;
                    section.data(6).dtTransOffset = 5;

                    ;% realtsim_P.Saturation_UpperSat
                    section.data(7).logicalSrcIdx = 33;
                    section.data(7).dtTransOffset = 6;

                    ;% realtsim_P.Saturation_LowerSat
                    section.data(8).logicalSrcIdx = 34;
                    section.data(8).dtTransOffset = 7;

                    ;% realtsim_P.Step2_Time
                    section.data(9).logicalSrcIdx = 35;
                    section.data(9).dtTransOffset = 8;

                    ;% realtsim_P.Step2_Y0
                    section.data(10).logicalSrcIdx = 36;
                    section.data(10).dtTransOffset = 9;

                    ;% realtsim_P.Step2_YFinal
                    section.data(11).logicalSrcIdx = 37;
                    section.data(11).dtTransOffset = 10;

                    ;% realtsim_P.realderivative1_A
                    section.data(12).logicalSrcIdx = 38;
                    section.data(12).dtTransOffset = 11;

                    ;% realtsim_P.realderivative1_C
                    section.data(13).logicalSrcIdx = 39;
                    section.data(13).dtTransOffset = 13;

                    ;% realtsim_P.realderivative2_A
                    section.data(14).logicalSrcIdx = 40;
                    section.data(14).dtTransOffset = 15;

                    ;% realtsim_P.realderivative2_C
                    section.data(15).logicalSrcIdx = 41;
                    section.data(15).dtTransOffset = 17;

                    ;% realtsim_P.Step3_Time
                    section.data(16).logicalSrcIdx = 42;
                    section.data(16).dtTransOffset = 19;

                    ;% realtsim_P.Step3_Y0
                    section.data(17).logicalSrcIdx = 43;
                    section.data(17).dtTransOffset = 20;

                    ;% realtsim_P.Step3_YFinal
                    section.data(18).logicalSrcIdx = 44;
                    section.data(18).dtTransOffset = 21;

            nTotData = nTotData + section.nData;
            paramMap.sections(4) = section;
            clear section


            ;%
            ;% Non-auto Data (parameter)
            ;%


        ;%
        ;% Add final counts to struct.
        ;%
        paramMap.nTotData = nTotData;



    ;%**************************
    ;% Create Block Output Map *
    ;%**************************
    
        nTotData      = 0; %add to this count as we go
        nTotSects     = 2;
        sectIdxOffset = 0;

        ;%
        ;% Define dummy sections & preallocate arrays
        ;%
        dumSection.nData = -1;
        dumSection.data  = [];

        dumData.logicalSrcIdx = -1;
        dumData.dtTransOffset = -1;

        ;%
        ;% Init/prealloc sigMap
        ;%
        sigMap.nSections           = nTotSects;
        sigMap.sectIdxOffset       = sectIdxOffset;
            sigMap.sections(nTotSects) = dumSection; %prealloc
        sigMap.nTotData            = -1;

        ;%
        ;% Auto data (realtsim_B)
        ;%
            section.nData     = 12;
            section.data(12)  = dumData; %prealloc

                    ;% realtsim_B.Step2
                    section.data(1).logicalSrcIdx = 0;
                    section.data(1).dtTransOffset = 0;

                    ;% realtsim_B.rads2rpm
                    section.data(2).logicalSrcIdx = 1;
                    section.data(2).dtTransOffset = 1;

                    ;% realtsim_B.pulse2deg
                    section.data(3).logicalSrcIdx = 2;
                    section.data(3).dtTransOffset = 2;

                    ;% realtsim_B.rads2rpm1
                    section.data(4).logicalSrcIdx = 3;
                    section.data(4).dtTransOffset = 3;

                    ;% realtsim_B.Gain
                    section.data(5).logicalSrcIdx = 4;
                    section.data(5).dtTransOffset = 4;

                    ;% realtsim_B.Sum
                    section.data(6).logicalSrcIdx = 5;
                    section.data(6).dtTransOffset = 5;

                    ;% realtsim_B.Step3
                    section.data(7).logicalSrcIdx = 6;
                    section.data(7).dtTransOffset = 6;

                    ;% realtsim_B.deg2rad1
                    section.data(8).logicalSrcIdx = 7;
                    section.data(8).dtTransOffset = 7;

                    ;% realtsim_B.deg2rad
                    section.data(9).logicalSrcIdx = 8;
                    section.data(9).dtTransOffset = 8;

                    ;% realtsim_B.Saturation
                    section.data(10).logicalSrcIdx = 9;
                    section.data(10).dtTransOffset = 9;

                    ;% realtsim_B.Gain_c
                    section.data(11).logicalSrcIdx = 10;
                    section.data(11).dtTransOffset = 10;

                    ;% realtsim_B.DiscreteTimeIntegrator
                    section.data(12).logicalSrcIdx = 11;
                    section.data(12).dtTransOffset = 11;

            nTotData = nTotData + section.nData;
            sigMap.sections(1) = section;
            clear section

            section.nData     = 1;
            section.data(1)  = dumData; %prealloc

                    ;% realtsim_B.AND
                    section.data(1).logicalSrcIdx = 12;
                    section.data(1).dtTransOffset = 0;

            nTotData = nTotData + section.nData;
            sigMap.sections(2) = section;
            clear section


            ;%
            ;% Non-auto Data (signal)
            ;%


        ;%
        ;% Add final counts to struct.
        ;%
        sigMap.nTotData = nTotData;



    ;%*******************
    ;% Create DWork Map *
    ;%*******************
    
        nTotData      = 0; %add to this count as we go
        nTotSects     = 4;
        sectIdxOffset = 2;

        ;%
        ;% Define dummy sections & preallocate arrays
        ;%
        dumSection.nData = -1;
        dumSection.data  = [];

        dumData.logicalSrcIdx = -1;
        dumData.dtTransOffset = -1;

        ;%
        ;% Init/prealloc dworkMap
        ;%
        dworkMap.nSections           = nTotSects;
        dworkMap.sectIdxOffset       = sectIdxOffset;
            dworkMap.sections(nTotSects) = dumSection; %prealloc
        dworkMap.nTotData            = -1;

        ;%
        ;% Auto data (realtsim_DW)
        ;%
            section.nData     = 1;
            section.data(1)  = dumData; %prealloc

                    ;% realtsim_DW.DiscreteTimeIntegrator_DSTATE
                    section.data(1).logicalSrcIdx = 0;
                    section.data(1).dtTransOffset = 0;

            nTotData = nTotData + section.nData;
            dworkMap.sections(1) = section;
            clear section

            section.nData     = 11;
            section.data(11)  = dumData; %prealloc

                    ;% realtsim_DW.EncoderInput_PWORK
                    section.data(1).logicalSrcIdx = 1;
                    section.data(1).dtTransOffset = 0;

                    ;% realtsim_DW.AnalogInput_PWORK
                    section.data(2).logicalSrcIdx = 2;
                    section.data(2).dtTransOffset = 1;

                    ;% realtsim_DW.Scope_PWORK.LoggedData
                    section.data(3).logicalSrcIdx = 3;
                    section.data(3).dtTransOffset = 2;

                    ;% realtsim_DW.Scope1_PWORK.LoggedData
                    section.data(4).logicalSrcIdx = 4;
                    section.data(4).dtTransOffset = 3;

                    ;% realtsim_DW.Scope2_PWORK.LoggedData
                    section.data(5).logicalSrcIdx = 5;
                    section.data(5).dtTransOffset = 6;

                    ;% realtsim_DW.wff_PWORK.LoggedData
                    section.data(6).logicalSrcIdx = 6;
                    section.data(6).dtTransOffset = 7;

                    ;% realtsim_DW.AnalogOutput_PWORK
                    section.data(7).logicalSrcIdx = 7;
                    section.data(7).dtTransOffset = 8;

                    ;% realtsim_DW.Scope_PWORK_f.LoggedData
                    section.data(8).logicalSrcIdx = 8;
                    section.data(8).dtTransOffset = 9;

                    ;% realtsim_DW.Scope1_PWORK_p.LoggedData
                    section.data(9).logicalSrcIdx = 9;
                    section.data(9).dtTransOffset = 10;

                    ;% realtsim_DW.Scope2_PWORK_m.LoggedData
                    section.data(10).logicalSrcIdx = 10;
                    section.data(10).dtTransOffset = 11;

                    ;% realtsim_DW.Scope3_PWORK.LoggedData
                    section.data(11).logicalSrcIdx = 11;
                    section.data(11).dtTransOffset = 12;

            nTotData = nTotData + section.nData;
            dworkMap.sections(2) = section;
            clear section

            section.nData     = 2;
            section.data(2)  = dumData; %prealloc

                    ;% realtsim_DW.StateSpaceController_SubsysRanBC
                    section.data(1).logicalSrcIdx = 12;
                    section.data(1).dtTransOffset = 0;

                    ;% realtsim_DW.EnabledSubsystem_SubsysRanBC
                    section.data(2).logicalSrcIdx = 13;
                    section.data(2).dtTransOffset = 1;

            nTotData = nTotData + section.nData;
            dworkMap.sections(3) = section;
            clear section

            section.nData     = 1;
            section.data(1)  = dumData; %prealloc

                    ;% realtsim_DW.StateSpaceController_MODE
                    section.data(1).logicalSrcIdx = 14;
                    section.data(1).dtTransOffset = 0;

            nTotData = nTotData + section.nData;
            dworkMap.sections(4) = section;
            clear section


            ;%
            ;% Non-auto Data (dwork)
            ;%


        ;%
        ;% Add final counts to struct.
        ;%
        dworkMap.nTotData = nTotData;



    ;%
    ;% Add individual maps to base struct.
    ;%

    targMap.paramMap  = paramMap;
    targMap.signalMap = sigMap;
    targMap.dworkMap  = dworkMap;

    ;%
    ;% Add checksums to base struct.
    ;%


    targMap.checksum0 = 1005310427;
    targMap.checksum1 = 1248552675;
    targMap.checksum2 = 3387333869;
    targMap.checksum3 = 3687229227;

